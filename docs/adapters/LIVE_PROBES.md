...
## Persistent activity order and direct row positions

On 2026-09-10, a live Codex run passed with 108 stored events, five messages and one tool. The verifier built and reopened ActivityIndex, compared all six visible row keys with reference viewer order and verified each direct position lookup. Paged state, browser checkpoint recovery, text pages, card projections, native resume and deduplication also passed. Combined filesystem state/index content used 246,334 bytes.

An initial native run timed out without stage attribution. Review identified redundant row-index I/O on text deltas; those existing-object transitions now advance only the sequence, with a regression that rejects any index read/write. The isolated native rerun passed without increasing its configured deadline. Added stage diagnostics reported 1,502 ms for content-store checks, 15,741 ms for server-snapshot checks and 17,317 ms for paged/index verification. These are diagnostic observations, not latency acceptance results or proof of a hard wall-clock deadline; JavaScript timeout timers remain best effort.

Full local validation passed 322 tests in 46 files. Persistent reducer/index checkpoint pairing, feed row/navigation adoption and long-session performance remain unfinished.

### Paired browser checkpoints — 2026-09-10

The installed Kimi probe completed two native turns with resume: 35 stored events, seven messages, and 19 activity rows. History backfill, live suffix capture, and publisher restart deduplication passed. Browser checkpoint reopen preserved content identity; all persisted browser row keys and positions matched the independent filesystem activity index and reference ordering. Seven paged text fields and 19 card projections matched. Stage observations: content verification 1,789ms, server snapshots 4,490ms, paged reduction 6,437ms. These are a small native probe, not long-session latency or memory acceptance. No native bodies or artifacts are retained here.

### Paged activity search and window loading — 2026-09-10

The installed Claude resume/import probe passed two native turns, 28 stored events, four messages and 16 activity rows. Two literal paged-search queries produced the same matching row identities and continuation as reference replay; the final loaded activity window matched persisted order. Four deferred text fields and 16 projections matched after browser checkpoint recovery. Import-to-live continuation, backfill, suffix capture and publisher restart deduplication passed. Stage observations: content 864ms, server snapshots 2,149ms, paged reduction/search/window verification 3,319ms. These checks use fake IndexedDB and native events, not browser UI interaction or long-session performance acceptance. No native bodies are retained in this report.

### Production paged browser sessions — 2026-09-10

The native verifier now opens the same BrowserPagedSession selected by the production application when saving is enabled. It receives the complete recording, verifies indexed rows and first/last text pages against reference replay, seeks to the midpoint, closes/reopens with the exact paused event boundary and speed, and returns to live. Claude passed with 28 events, 16 activity rows and 12 text-field checks across those presentations (229ms for this additional stage). Kimi passed with 35 events, 19 activity rows and 22 text-field checks (409ms for this stage). Both native resume and publisher restart deduplication passed. These small probes use fake IndexedDB with actual server transport and native-agent sessions; they do not prove browser rendering, device behavior, long-session latency or heap limits. Aggregate evidence only; no private transcript bodies are retained here.

### Local seek catalogs — 2026-09-10

The installed Kimi resume probe passed 35 stored events and 19 activity rows. The paged verifier reopened local state and sought from saved sequence 1 rather than replaying the entire prefix; the resulting sequence and row order matched reference playback. The production session also passed explicit seek, paused-presentation reopen and return to live, with 22 text-field checks across those presentations. Stage observations: content 1,814ms, server snapshots 4,367ms, paged verification 6,345ms, production session 318ms. These small native probes do not establish long-session latency or memory bounds.

The OpenCode three-turn resume/import probe passed 26 events, six messages and one attachment, including native-server restart, detached history recovery, resumed import, and publisher restart deduplication. Production paged receipt/seek/reopen passed with seven activity rows and 16 text-field checks. The separate retained-history paged-reducer run passed all 12 OpenCode exports (129 events, 29 messages); one historical artifact remained explicitly unavailable. That corpus run verifies paged reconstruction, not browser interaction or the seek-catalog performance target. Only aggregates are recorded here.

### Paired server snapshot activity — 2026-09-10

The installed Claude resume/import probe passed 28 events and 16 activity rows. The shared snapshot client verified all row identities and positions against reference replay through the authorized server content API, in addition to full state equivalence. The range cache reopened with zero additional content downloads; the complete first pass used 60 content reads. Stage observations: content 1,268ms, server snapshot/state/activity verification 5,054ms, paged verification 4,906ms, production paged session 192ms. Native resume, import continuation, suffix capture and publisher restart deduplication passed. A separate filesystem regression verifies that an agent first mentioned before its own creation retains its earlier ordering when generation resumes from a paired checkpoint. These checks do not establish browser interaction or long-session performance acceptance.
