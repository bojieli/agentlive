...
`provenance.completenessNotice` is the effective `capture.completeness` notice at the frozen boundary: the latest notice after any `recording.reopened`, with its server sequence as `at`. It contains counts and a fixed reason only. It is omitted when none applies. Readers recompute it from `events.jsonl` and reject a missing, extra or different value. Archives written before the field existed cannot contain the event, so they remain valid version 1 archives. Replay derives the notice from events regardless of the manifest.
