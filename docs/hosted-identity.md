...
### Disabling suspends the account's recordings

Disabling is the moderation control for an abusing account, so it suspends every recording the account owns, in both directions, until the account is enabled again. It is the reversible counterpart of removal, which is a permanent per-recording tombstone.

- **Writing stops.** A recording whose owner account is disabled accepts no publishing: `resume`, event batches, `end`, `reopen` and attachment upload all fail with protocol error code `forbidden` (HTTP 403, or a WebSocket `error` message). The check is independent of the credential presented, including the recording's own write secret, so a running publisher cannot outlive the disable. `forbidden` is not retryable, so the publisher and the attachment uploader stop with that error instead of spinning; unacknowledged events stay in the local publisher journal. An open publisher socket is closed with code 1008 at the moment the disable commits, alongside the viewing sockets. Creating or importing a recording already fails earlier, because the account's browser and device credentials stop authenticating.
- **Reading stops.** The account's recordings are not served and not listed while it is disabled, whatever their visibility: direct metadata, history, export, snapshot, attachment and watch-ticket requests return `forbidden`, `GET /api/v1/public-recordings` omits them, and a watch socket that subscribes (or is already subscribed) to one is refused or closed with code 1008. Anonymous public reads already in flight are cancelled the same way a public-to-private visibility change cancels them; reads authorized by other credentials are cancelled through the transfer registry. Bytes already delivered cannot be withdrawn.
- **The operator still reads.** The owner credential bypasses the suspension for reads, so an operator can review the reported content before deciding between re-enabling and removal. Operator listing, metrics and removal are unaffected.
- **Enabling restores both.** Publishing resumes for any recording that was not otherwise ended, reads and public listing come back, and the recordings' own content and visibility are unchanged. Credentials issued before the disable stay invalid, so the publisher must hold the recording write secret (or the user must sign in again).
- **Nothing else is affected.** Other accounts, the local owner (`ownerId` `local`, used by the operator credential) and every recording on a standalone server without hosted accounts behave exactly as before; neither has an account record to disable. Server-built snapshots are derived data and keep converging in the background; they are not served while the account is disabled.

The gate reads the in-memory account ledger, so it costs a map lookup per published event, read and listed recording, and never touches disk. Tests: `tests/recovery/account-suspension.test.ts` over real HTTP with the hosted fixture, covering the closed publisher socket, refused resume/append/attachment/lifecycle, suspended direct reads, listing, watch ticket and sockets, operator review, unaffected second account and local owner, restored publishing and serving after enabling, and an unchanged standalone server.
