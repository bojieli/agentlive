...
Bytes already written to the socket, the kernel buffers or the client cannot be withdrawn. An import or upload that commits before the revocation is processed stays committed. The static operator credential is not revocable at runtime. WebSocket output still uses its existing send-time and heartbeat checks rather than this registry.
