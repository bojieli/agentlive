...
The ledger is a private server file under the exclusive server lock. Writes commit before acknowledgement; uncertain storage failures require reopening. Input and persisted reads are bounded.

Intake is partitioned so one recording cannot consume it. The limits are 16 reports awaiting review **per recording**, 500 awaiting review in total, 32 new reports per minute across the server, 32 queued operations, and 1,000 reports in the ledger. Existing identical retries consume no new-report capacity, and the minute limit resets on restart. Reports an operator has already dismissed or removed never count against intake: when the ledger is full the oldest of them make room for a new report, and the number dropped that way is durable and returned with every operator listing as `droppedReviewed`, so a trimmed ledger is never silent. At capacity the server returns `retry_later`. Report archival to durable storage, per-reporter limits and operational capacity management remain unfinished; anonymous reporters behind one proxy are not told apart.
