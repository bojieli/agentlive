...
This is the writable content backend. BrowserPagedState now uses its atomic checkpoint pointer for completed working-state publication; see [BROWSER_PAGED_STATE.md](BROWSER_PAGED_STATE.md). Production BrowserSession still needs to adopt it along with history ranges and asynchronous viewport queries. The existing range cache serves read-only server snapshot ranges; it does not automatically import entire immutable blobs into this store. That transport/local-content bridge and safe root retention are still required for snapshot-plus-suffix reconstruction in the browser.
