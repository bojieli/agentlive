...
What is not included or not paused: derivative snapshots and snapshot leases (restore removes them anyway) and in-flight upload staging (discarded on reopen) are skipped, so snapshot building, lease renewal and upload body streaming keep running; an upload only waits for its final installation step. Automatic snapshot collection is the one derivative operation that does take the barrier as a shared holder, so that it cannot delete derived files in the middle of the barrier phase: a pass already running cancels itself as soon as a backup asks for the barrier (deleting nothing further and leaving every retained root readable), and a pass requested while a backup is pending is refused with a retryable error instead of queueing behind it. A backup therefore never waits for a collection pass beyond its cancellation, and the two cannot deadlock. The online backup always records the running server's owner credential as `owner.json`, whatever its source (including an environment-provided secret).
...
The source is the specific hashed directory containing `binding.json` and the journal files (`capture.jsonl`, plus `journal.json` and `capture-<n>.jsonl` once the journal has rotated), normally under the publisher's AgentLive state directory at `publisher/<hash>`. Use the existing directory for that native session; no new binding is created. Recovery takes the publisher lock and uses its stored write credential. It requires a changed server revision and matching stream/publisher/epoch identity, and it verifies the restored server's entire publisher prefix against the local journal. Divergent or missing local history is rejected.

### What a pruned journal can still verify

A long-running live file publisher (Claude, Codex, Kimi) keeps its journal bounded: once the server durably acknowledges events, their capture records are compacted away and only a running hash chain over those events, their source-key index and the adapter checkpoint at the boundary are kept. `agentlive publications` and the journal manifest report that boundary as the compacted producer sequence.

Recovery still verifies the whole restored prefix, in two ways:

- Events at or before the compaction boundary are verified **collectively**, by recomputing the publisher hash chain over the restored server's events and comparing it with the chain stored in `journal.json`. Producer epoch, recording ID and sequence continuity are still checked per event. Any difference in content, ordering or count anywhere inside that region changes the chain and is rejected as `event_conflict`. What recovery cannot do for this region is say _which_ event differs, or reproduce the local copy of it — those records no longer exist locally.
- Events after the compaction boundary are verified **event by event** against the retained records, exactly as before, and are the events recovery can republish.

Two consequences follow. A restored prefix that ends _inside_ the compacted region cannot be verified at all — there is no chain checkpoint at that sequence — so recovery refuses with `sequence_gap` and changes nothing; restore a newer backup, or accept the recording as it stands. And attachments referenced only by compacted events are not re-uploaded, because their events are already durable on the server side of the boundary; only the suffix's attachments are.
...
An ended recording remains ended and needs the existing explicit reopening workflow. Changed server origins, publisher epoch handoff, import-origin converter/filter upgrades and incomplete artifact spools are not automatically migrated by this command. The local recovery tests publish beyond a backup, restore the older prefix, recover through the CLI and verify exact suffix redelivery with its attachment, and separately cover a pruned journal: a chain-verified prefix recovers, a prefix ending inside the compacted region is refused, and a journal whose compacted history differs is rejected. Full multi-adapter/crash/deployment acceptance remains open.
