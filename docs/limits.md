...
- **The server's fan-out knee is between about 51,000 and 60,000 event deliveries per second** (98–116 MB/s), with the server process at roughly one saturated core. Past it the server queues rather than sheds: at 240 viewers and 4,800 events/s offered, 55% of expected deliveries happened inside the window and p50 latency reached 5.5 s. No `retry_later`, socket close or viewer reconnect occurred at any shape — the measurement above predates the overload signal.
- Since that measurement the server reports the state instead of hiding it. A mean event-loop delay above `--overload-event-loop-delay-ms` (default 250 ms), or more than 64 MiB queued across all socket send buffers, makes `/readyz` return 503 with `overloaded`, sets `agentlive_delivery_overloaded` and refuses *new* viewer connections with `retry_later`. Existing viewers keep their sockets, because dropping them converts a latency problem into a reconnect storm, and publishers are never refused. The signal holds for five seconds after the measurement recovers so admission does not flap. This bounds how many viewers a saturated server accepts; it does not raise the knee, and the rows above have not been re-measured with it in place.
