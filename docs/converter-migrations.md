...
- Artifact capture pins its redaction rule in `policy.json` inside the binding's artifact capture directory, independently of the converter version and filter fingerprint. Rule 1 filtered only artifacts whose caller declared them text; rule 2 decides from the captured bytes (strict UTF-8 decode, otherwise a byte-level scan for each secret's UTF-8 encoding) and is what new bindings pin. A directory that already holds captures keeps rule 1 on upgrade, because its request bindings resolve to bytes produced under that rule; rule 2 additionally names itself in every new request hash, so the two cannot be confused. Opening a pinned directory under the other rule is rejected. Replacement migration creates a new binding and therefore captures under the installed rule.
