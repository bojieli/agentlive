...
Audited 2026-09-09 against implementation plan sections 6–15, current source, CLI commands, and probe evidence. Server snapshot changes have local validation; The preceding index checkpoint `c49779d` failed macOS CI on a nondeterministic unfinished OpenCode fixture; the current work explicitly marks that terminal fixture and adds deterministic active-tail coverage. CI must be checked on the new head. This inventory preserves the full M0–M7 objective. Historical checkpoint notes are evidence of individual changes, not proof that an entire milestone is complete.

Current local validation passes 290 tests in 37 files and standalone package installation/rebuild verification. Native import/live/recovery probes cover all four agents across the recorded work; the latest Codex probe additionally verifies incremental text references, reopened indexed reads and shared-client snapshot reconstruction. CI status and its fixture correction are distinguished above from these local results. The latest corpus run passed 2,564 of 2,599 examined files; 35 Claude files lacked native identity/timestamps. Successful conversions still contain explicit capture gaps and unavailable artifacts. Test counts establish checkpoint behavior, not milestone completion.

## 1. Scalable playback and complete viewers — M1/M4

- Integrate immutable disk-backed content pages and the new persistent key index with a paged reducer; keep completed text outside the active heap. Insertion-order maps and reducer use of incremental text remain required; TextStore now appends by reusing complete pages.
