...
3. **Hosted authorization follow-ups.** Close WebSockets promptly on account/device revocation (they currently fail at their next send-time or heartbeat check); add an operator account-disable route with HTTP-level tests; per-account quotas; real identity-provider acceptance.
4. **Operations.** Recording-format upgrade and tested rollback automation. Online backup is implemented; exercise it in the container probe and on an actual host.
