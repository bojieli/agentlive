#!/usr/bin/env node
/** Local structural conversion validation of every discoverable Claude history, without publication. */
...
const output = resolve("probe-results", "claude-history-validation");
...
const results = [];
for (const root of [join(homedir(), ".claude", "projects")])
  for await (const path of files(root)) {
    ...
    try {
      ...
      const report = await captureClaudeHistory(path, manifest, sink);
      result.events = sink.capturedThrough;
      result.unsupportedItemTypes = report.unsupported;
      ...
      summary.converted++;
      for (const [type, count] of Object.entries(report.unsupported))
