#!/usr/bin/env node
/** Local structural conversion validation of every discoverable Kimi history, without publication. */
...
const output = resolve("probe-results", "kimi-history-validation");
...
const results = [];
for (const root of [join(homedir(), ".kimi-code", "sessions")])
  for await (const path of files(root)) {
    ...
    try {
      ...
      const report = await captureKimiHistory(path, manifest, sink);
