...
- 24 offline tests pass locally, including actual child-process SIGKILL and SIGSTOP scenarios. Command: `npx --yes pnpm@12.3.4 check`.
...
| M2 server and first vertical slice | Not implemented | Idempotent stream creation, credentials, durable session writer/leases, append deduplication, HTTP JSONL boundaries, WebSocket synchronization, attachment API, CLI and minimal viewers, real Codex publish-to-replay. |
...
The publisher journal retains its captured history up to a configured byte limit; it does not prune acknowledged segments yet. Capturing before the first remote binding is established is not implemented. Attachment references are modeled but attachment storage/upload is not implemented. No server or viewer currently exposes these modules to external users. File-lock prebuild installation is verified locally on macOS/Node 26; Linux coverage belongs to the CI run. Source/session recovery behavior of each actual agent is not yet established by the short streaming probes.
