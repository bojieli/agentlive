...
With Save history enabled at join, the browser uses a paged IndexedDB content store and atomically publishes reducer state and activity ordering. Reload authenticates first and continues from the saved receipt cursor. The feed loads indexed row windows, card metadata and bounded text ranges instead of retaining the complete event array. Seeking downloads fixed-boundary history suffixes and reconstructs a separate immutable presentation; receipt continues independently. Backward seeking can still require replay from the beginning, and historical seeking currently requires server access. The content store has a 512 MiB encoded-blob quota; content collection and measured long-session heap/seek limits remain unfinished. A storage or concurrent-writer conflict reports an error requiring rejoin/recovery rather than silently changing the durable cursor. With saving disabled or IndexedDB unavailable, the reference memory viewer retains its 64 MiB encoded-event limit. Clear saved histories closes the current viewer and removes local copies without deleting server recordings. Access keys and attachment bytes are not persisted by this browser content store. Hidden pages suspend receipt; foreground recovery revalidates access and catches up without moving a paused view. Actual desktop/mobile storage, lifecycle and visual verification remain release work.
